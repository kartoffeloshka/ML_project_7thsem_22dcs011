...
def plot_dendrogram(features_to_cluster, n_true_clusters):
    ...
    plt.axhline(y=plt.gca().get_ylim()[1]*0.7, color='r', linestyle='--', label=f'Cut for k={n_true_clusters}')
    ...
    plt.tight_layout()
    plt.savefig('breast_cancer_dendrogram.png')
    print("Saved 'breast_cancer_dendrogram.png'. Check this plot to help choose 'k'.")
...
def plot_results(pca_df):
    ...
    plt.suptitle('Breast Cancer Subtype Clustering: Ground Truth vs. Hierarchical', fontsize=16, y=1.02)
    
    plt.tight_layout()
    plt.savefig('breast_cancer_hierarchical_results.png')
    print("Saved 'breast_cancer_hierarchical_results.png'.")
