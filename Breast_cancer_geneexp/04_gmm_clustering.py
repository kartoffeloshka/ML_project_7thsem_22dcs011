...
def plot_gmm_metrics(features_to_cluster, max_k=10):
    ...
    plt.title('GMM Model Selection (Breast Cancer)')
    plt.xlabel('Number of Clusters (k)')
    plt.ylabel('Information Criterion Value')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.savefig('breast_cancer_gmm_aic_bic.png')
    print("Saved 'breast_cancer_gmm_aic_bic.png'. The 'elbow' (minimum) suggests the best k.")
...
def plot_results(pca_df):
    ...
    plt.suptitle('Breast Cancer Subtype Clustering: Ground Truth vs. GMM', fontsize=16, y=1.02)
    
    plt.tight_layout()
    plt.savefig('breast_cancer_gmm_results.png')
    print("Saved 'breast_cancer_gmm_results.png'.")
