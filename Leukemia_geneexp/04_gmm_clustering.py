...
# --- MODIFIED IMPORTS ---
from sklearn.mixture import GaussianMixture # <-- This is the new method
...
import warnings
import numpy as np
from sklearn.exceptions import ConvergenceWarning  # <-- ADD THIS LINE
...
def load_and_prepare_data(filepath='Leukemia_GSE9476.csv'):
    ...
    # Drop the 'samples' column (specific to this dataset)
...
# --- NEW FUNCTION (Replaces Elbow/Dendrogram) ---
def plot_gmm_metrics(features_to_cluster, max_k=10):
    """
    Calculates and plots the AIC and BIC for GMM with different 'k' values.
    The 'optimal' k is the one that minimizes AIC or BIC.
    """
    ...
    plt.title('GMM Model Selection (Leukemia)')
    plt.xlabel('Number of Clusters (k)')
    plt.ylabel('Information Criterion Value')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.savefig('leukemia_gmm_aic_bic.png')
    print("Saved 'leukemia_gmm_aic_bic.png'. The 'elbow' (minimum) suggests the best k.")
# --- END NEW FUNCTION ---
...
def perform_gmm_clustering(features_to_cluster, pca_df_2d, true_labels, n_clusters):
    """
    Performs Gaussian Mixture Model (GMM) clustering and evaluates the results.
    """
    print(f"\nPerforming GMM clustering with k={n_clusters}...")
    
    # --- Clustering ---
    # We cluster on the high-dimension PCA data.
    # n_components is the GMM term for 'n_clusters'
    gmm_model = GaussianMixture(
        n_components=n_clusters, 
        random_state=42,
        n_init=10  # Run 10 times with different seeds
    )
    # Use fit_predict() to get hard cluster assignments
...
def plot_results(pca_df):
    ...
    2. PCA colored by GMM predicted cluster.
    """
    ...
    plt.suptitle('Leukemia Subtype Clustering: Ground Truth vs. GMM', fontsize=16, y=1.02)
    # --- END MODIFIED TITLES ---
    
    plt.tight_layout()
    # --- MODIFIED FILENAME ---
    plt.savefig('leukemia_gmm_results.png')
    print("Saved 'leukemia_gmm_results.png'.")
