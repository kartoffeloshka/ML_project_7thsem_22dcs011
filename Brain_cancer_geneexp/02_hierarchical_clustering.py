...
def plot_dendrogram(features_to_cluster, n_true_clusters):
    ...
    plt.title('Hierarchical Clustering Dendrogram (Brain Tumor)')
    plt.xlabel('Patient Samples or Clusters (Truncated)')
    plt.ylabel('Distance (Ward Linkage)')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6, axis='y')
    plt.tight_layout()
    plt.savefig('brain_cancer_dendrogram.png')
    print("Saved 'brain_cancer_dendrogram.png'. Check this plot to help choose 'k'.")
...
def plot_results(pca_df):
    ...
    plt.suptitle('Brain Tumor Subtype Clustering: Ground Truth vs. Hierarchical', fontsize=16, y=1.02)
    plt.tight_layout()
    plt.savefig('brain_cancer_hierarchical_results.png')
    print("Saved 'brain_cancer_hierarchical_results.png'.")
