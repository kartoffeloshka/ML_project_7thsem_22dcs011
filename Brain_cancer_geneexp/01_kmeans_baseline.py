...
def find_optimal_k(features_to_cluster, max_k=10):
    ...
    plt.title('Elbow Method for Optimal k (Brain Tumor)')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.savefig('brain_cancer_elbow_plot.png')
    print("Saved 'brain_cancer_elbow_plot.png'. Check this plot to help choose 'k'.")
...
def perform_clustering(features_to_cluster, pca_df_2d, true_labels, n_clusters):
    ...
    print(f"3. Homogeneity:                   {homogeneity:.4f} (Good: ~1.0)")
    print(f"4. Completeness:                  {completeness:.4f} (Good: ~1.0)")
    print(f"5. V-measure (Homogeneity & Comp.): {v_measure:.4f} (Good: ~1.0)")
    ...
    print(f"6. Silhouette Score:              {silhouette:.4f} (Good: +1, Bad: -1)")
    print(f"7. Davies-Bouldin Score:          {db_score:.4f} (Good: 0.0, Bad: higher)")
...
def plot_results(pca_df):
    ...
    plt.suptitle('Brain Tumor Subtype Clustering: Ground Truth vs. K-Means', fontsize=16, y=1.02)
    plt.tight_layout()
    plt.savefig('brain_cancer_clustering_results.png')
    print("Saved 'brain_cancer_clustering_results.png'.")
